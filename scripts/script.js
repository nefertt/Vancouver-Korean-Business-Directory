...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const translations = {
    ...
    en: {
      ...
      heroSubtitle:
        "Restaurants, markets, hospitals, real estate - all your life information in one place.",
  ...
  ctaBtn.addEventListener("click", function () {
    ...
    alert(
      "내 주변 한인 업체를 찾는 기능입니다. 실제로는 위치 기반 검색이 실행됩니다."
    );
  });
  ...
  categoryCards.forEach((card) => {
    card.addEventListener("click", function () {
      ...
      alert(
        `${category} 카테고리를 선택했습니다. 실제로는 해당 카테고리 페이지로 이동합니다.`
      );
    });
  });
  ...
  setTimeout(() => {
    const businessCards = document.querySelectorAll(
      ".business-card:not(.featured-card)"
    );
    businessCards.forEach((card) => {
      card.addEventListener("click", function () {
        ...
        alert(
          `${businessName} 업체를 선택했습니다. 실제로는 업체 상세 페이지로 이동합니다.`
        );
      });
    });
