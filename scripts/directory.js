...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const sidebarCheckboxes = document.querySelectorAll(".checkbox-filters input[type='checkbox']");
  ...
  function filterAndRender() {
    ...
    if (currentFilters.area !== 'all') {
      filteredData = filteredData.filter((business) => business.area === currentFilters.area);
    }
  ...
  function updateFiltersFromSidebar() {
    const selectedCategories = Array.from(
      document.querySelectorAll('.checkbox-filters input[data-category]:checked'),
    ).map((cb) => cb.dataset.category);

    const selectedAreas = Array.from(document.querySelectorAll('.checkbox-filters input[data-area]:checked')).map(
      (cb) => cb.dataset.area,
    );
    ...
    if (selectedCategories.length === 0 || selectedCategories.length === 7) {
      ...
      document.querySelector(`.filter-chip[data-category="${selectedCategories[0]}"]`).classList.add('active');
    }
    ...
    businessCardsContainer.innerHTML = businesses
      ...
      <div class="business-card" data-business-id="${business.id}">
        ...
        <div class="business-actions">
          <button class="action-btn" onclick="viewOnMap(${business.id})">지도에서 보기</button>
          <button class="action-btn secondary" onclick="viewDetails(${business.id})">자세히 보기</button>
        </div>
      </div>
    `,
      )
  ...
  window.viewOnMap = function (businessId) {
    ...
    alert(`${business.name}을(를) 지도에서 보기 기능입니다. 실제로는 지도 페이지로 이동합니다.`);
  };
  ...
  if (loadMoreBtn) {
    loadMoreBtn.addEventListener('click', function () {
      ...
      alert('더 많은 업체를 불러오는 기능입니다. 실제로는 서버에서 추가 데이터를 가져옵니다.');
    });
  }
  ...
  searchInput.addEventListener('input', debounce(filterAndRenderWithLoading, 300));
