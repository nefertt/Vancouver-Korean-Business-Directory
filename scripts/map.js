...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderListPanels(businesses) {
    const listHTML = businesses
      .map(
        (business) => `
        <div class="list-item" data-business-id="${
          business.id
        }" onclick="selectBusinessFromList(${business.id})">
  ...
  window.selectBusinessFromList = function (businessId) {
    ...
    const marker = markers.find((m) => {
      const business = businessData.find((b) => b.id === businessId);
      return (
        m.getLatLng().lat === business.coordinates[0] &&
        m.getLatLng().lng === business.coordinates[1]
      );
    });
  ...
  listToggleBtn.addEventListener("click", function () {
    if (window.innerWidth > 1024) {
      ...
      this.textContent = desktopListPanel.classList.contains("visible")
        ? "지도만 보기"
        : "리스트 보기";
    } else {
      // Mobile: toggle bottom sheet
      mobileBottomSheet.classList.toggle("visible");
      this.textContent = mobileBottomSheet.classList.contains("visible")
        ? "지도만 보기"
        : "리스트 보기";
    }
  });
  ...
  function updateContent(lang) {
    ...
    const isListVisible =
      desktopListPanel.classList.contains("visible") ||
      mobileBottomSheet.classList.contains("visible");
    listToggleBtn.innerHTML = `<span class="btn-icon">📋</span>${
      isListVisible ? content.mapOnly : content.listView
    }`;
    ...
    // Update list headers
    const listHeaders = document.querySelectorAll(
      ".list-header h3, .sheet-header h3"
    );
